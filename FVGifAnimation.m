...
+ (BOOL)canAnimateImageSource:(CGImageSourceRef)imgSrc{
    ...
    if([(NSString*)type isEqual:@"com.compuserve.gif"]){
    ...
    for(size_t i=0;i<imageCount;i++){
        ...
        if(propI){
            ...
            if(gifProp){
                ...
                delay=[(NSNumber*)delayVal doubleValue];
            }
            CFRelease(propI);
        }
...
- (NSUInteger)gcdInNumbers:(NSIndexSet*)set{
    NSMutableIndexSet* delays=[[set mutableCopy] autorelease];
    ...
    if(imgSrc && [[self class] canAnimateImageSource:imgSrc]){
        ...
        if([(NSString*)type isEqual:@"com.compuserve.gif"]){
            ...
            if(gifProp){
                ...
                loopCount=[(NSNumber*)loopNum integerValue];
            }
            CFRelease(prop);
            NSArray* delays=[self delaysForImageSource:imgSrc];
            self->duration=[self totalDurationForDelays:delays];
            self->images=[[self animationImagesForImageSource:imgSrc withDelays:delays] retain];
